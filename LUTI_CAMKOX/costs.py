...
from LUTI_CAMKOX.globals import modelRunsDir, QUANTCijRoadCentroidsFilename, QUANTCijBusCentroidsFilename, QUANTCijRailCentroidsFilename
...
def costMSOAToPoint_3modes(cij_roads, cij_bus, cij_rail, dfPoints, CAMKOX_MSOA_list):
    ...
    # Filter out CAMKOXordshire from EWS files
    df_roads = df_roads.loc[CAMKOX_MSOA_list]  # Filter rows
    df_bus = df_bus.loc[CAMKOX_MSOA_list]  # Filter rows
    df_rail = df_rail.loc[CAMKOX_MSOA_list]  # Filter rows
    ...
    cijpoint_roads = np.zeros(m * p, dtype=float).reshape(m, p)  # so m=MSOA and p=points index
    cijpoint_bus = np.zeros(m * p, dtype=float).reshape(m, p)  # so m=MSOA and p=points index
    cijpoint_rail = np.zeros(m * p, dtype=float).reshape(m, p)  # so m=MSOA and p=points index
